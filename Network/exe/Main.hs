...
import           Cantis.Crypto.Utils.Keys.Signature
import           Cantis.Crypto.Utils.PublicKey.Utils
import           Cantis.Env
import           Cantis.Utils.Logging
import           Cantis.Network.Connection           (CompleteConnection,
                                                     ConnectionId)
import           Cantis.Network.Instance
import           Cantis.Network.StreamServer
import qualified Cantis.Network.Types                as ANT (PersonalityType (INITIATOR),
                                                            TransportType (TCP))
...
type AppM = ReaderT CantisEnv (LoggingT IO)

instance HasEnv AppM where
  getEnv = ask

instance HasCantisNetworkInstance AppM where
  getcantisNetworkInstance = cantisNetworkInstance <$> getEnv

instance HasSecretKey AppM where
  getSecretKey = cryptoEnvSercretKey . cantisCryptoEnv <$> getEnv
...
sender sk rk n size = do
  ...
  -- sock <- createUDPSocket "127.0.0.1" (envPort mkCantisEnv)
...
receiver sk = do
  ...
  mutableConnectionHashMap1 <- MutableHashMap.new
                                    :: IO (HashTable ConnectionId CompleteConnection)
  env' <- mkCantisEnv
  let env = env' { cantisCryptoEnv = CryptoEnv sk
                 , loggerChan = tq
                 -- , udpSocket = sock
